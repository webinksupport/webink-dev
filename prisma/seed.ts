...
const products: ProductSeed[] = [
  ...
  // standalone simple product at $493 (appears to be a one-time purchase version)
  {
...
async function main() {
  console.log('🔄 Rebuilding products from WooCommerce export...')
  console.log('   Deleting existing products and variants...')

  // Delete all existing variants first (FK constraint), then products
  ...
  // ─── Re-seed navigation menu (unchanged) ───────────────────────────────
  console.log('\nSeeding default navigation menu...')
  await prisma.menuItem.deleteMany({ where: { menuSlug: 'primary' } })

  const home = await prisma.menuItem.create({ data: { label: 'Home', url: '/', order: 0, menuSlug: 'primary' } })
  const servicesMenu = await prisma.menuItem.create({ data: { label: 'Services', url: '/services', order: 1, menuSlug: 'primary' } })
  const pricing = await prisma.menuItem.create({ data: { label: 'Pricing', url: '/pricing', order: 2, menuSlug: 'primary' } })
  const about = await prisma.menuItem.create({ data: { label: 'About', url: '/about', order: 3, menuSlug: 'primary' } })
  const contact = await prisma.menuItem.create({ data: { label: 'Contact', url: '/contact', order: 4, menuSlug: 'primary' } })
