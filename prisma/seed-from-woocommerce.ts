...
const products: ProductSeed[] = [
  ...
  // variable-subscription: 3 levels × 2 renewal = 6 variations
...
async function main() {
  ...
  const postSubs = await prisma.subscription.count()
  console.log(`\n📊 Post-seed safety verification:`)
  console.log(`   Users: ${postUsers} (was ${preUsers}) ${postUsers === preUsers ? '✓' : '⚠ MISMATCH'}`)
  console.log(`   Orders: ${postOrders} (was ${preOrders}) ${postOrders === preOrders ? '✓' : '⚠ MISMATCH'}`)
  console.log(`   Subscriptions: ${postSubs} (was ${preSubs}) ${postSubs === preSubs ? '✓' : '⚠ MISMATCH'}`)

  // ─── Re-seed navigation menu ────────────────────────────────────────
  console.log('\nSeeding default navigation menu...')
  await prisma.menuItem.deleteMany({ where: { menuSlug: 'primary' } })

  const servicesMenu = await prisma.menuItem.create({ data: { label: 'Services', url: '/services', order: 1, menuSlug: 'primary' } })
  await prisma.menuItem.create({ data: { label: 'Home', url: '/', order: 0, menuSlug: 'primary' } })
  await prisma.menuItem.create({ data: { label: 'Pricing', url: '/pricing', order: 2, menuSlug: 'primary' } })
  await prisma.menuItem.create({ data: { label: 'About', url: '/about', order: 3, menuSlug: 'primary' } })
  await prisma.menuItem.create({ data: { label: 'Contact', url: '/contact', order: 4, menuSlug: 'primary' } })

  const serviceChildren = [
    { label: 'Web Design', url: '/services/web-design', order: 0 },
    { label: 'SEO', url: '/services/seo', order: 1 },
    { label: 'Social Media', url: '/services/social-media', order: 2 },
    { label: 'Paid Advertising', url: '/services/paid-advertising', order: 3 },
    { label: 'AI Marketing', url: '/services/ai-marketing', order: 4 },
    { label: 'Custom CRM', url: '/services/custom-crm', order: 5 },
    { label: 'Web Hosting', url: '/services/web-hosting', order: 6 },
  ]
  ...
  console.log('  ✓ Primary navigation menu seeded')
  console.log('\n🎉 Seeding complete!')
}
