    ...
    <section id="services" className="py-24 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="grid lg:grid-cols-2 gap-12 mb-16 lg:mb-20"
        >
          <div>
            <div className="flex items-center gap-3 mb-5">
              <span className="inline-block w-6 h-px bg-[#F813BE]" />
              <span className="text-xs font-bold tracking-[0.35em] text-black/30 uppercase font-urbanist">What We Do</span>
            </div>
            <h2 className="font-urbanist font-black text-[#0A0A0A] leading-tight" style={{ fontSize: 'clamp(2.2rem, 4vw, 3.5rem)' }}>
              Full-Service<br />
        ...
        >
          {services.map((svc) => (
            <motion.div key={svc.title} variants={item}>
              <Link
                href={svc.href}
                className="group relative flex flex-col bg-white hover:bg-[#F5F5F5] transition-all duration-300 p-8 lg:p-10 h-full"
              >
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="font-urbanist text-4xl font-black text-black/5 group-hover:text-black/10 transition-colors">
                    {svc.tag}
                  </span>
                </div>

                <h3 className="font-urbanist text-xl font-bold text-[#0A0A0A] mb-3">
                  {svc.title}
                </h3>
                <p className="text-black/45 text-sm leading-relaxed font-urbanist mb-6 flex-1">
                  {svc.desc}
                </p>

                <div
                  className="text-xs font-bold flex items-center gap-1.5 translate-y-1 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 font-urbanist"
        ...
        >
          <p className="text-black/30 text-sm font-urbanist">Not sure what you need? Let&apos;s talk.</p>
          <Link
            href="/contact"
            className="text-sm font-bold text-black border-b-2 border-[#14EAEA] pb-0.5 hover:text-[#14EAEA] transition-colors duration-200 font-urbanist"
          >
